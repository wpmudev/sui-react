import { Section, Switcher, Preview, Snippet, Code } from "@wpmudev/sui-docs"
import { SettingBlock } from "../../src"
import dedent from "dedent"

export const generalExample = dedent`
    <SettingBlock
      title="Title"
      description="Block IP addresses that send POST requests with empty Referer and User-Agent headers."
    >
       The Setting Block Content
    </SettingBlock>
`

export const proExample = dedent`
    <SettingBlock
      isPro={true}
      title="Title"
      description="Block IP addresses that send POST requests with empty Referer and User-Agent headers."
    >
       The Setting Block Content
    </SettingBlock>
`

<Section style={{ paddingTop: 30 }}>
  <p>Below is a general example of the Setting Block component</p>

  <Preview wrapper={true}>
    <div style={{ background: "#f1f1f1", padding: 25 }}>
      <div style={{ background: "white" }}>
        <SettingBlock title="Title" description="Block IP addresses that send POST requests with empty Referer and User-Agent headers.">
          The Setting Block Content
        </SettingBlock>
      </div>
    </div>
  </Preview>

  <Snippet language="js">
    {generalExample}
  </Snippet>
</Section>

<Section title="Pro variation" style={{ paddingTop: 30 }}>
  <p>
    Set the prop <Code small={true}>isPro=\{true}</Code> to change the variation to the Pro variation
  </p>

  <Preview wrapper={true}>
    <div style={{ background: "#f1f1f1", padding: 25 }}>
      <div style={{ background: "white" }}>
        <SettingBlock isPro={true} title="Title" description="Block IP addresses that send POST requests with empty Referer and User-Agent headers.">
          The Setting Block Content
        </SettingBlock>
      </div>
    </div>
  </Preview>

  <Snippet language="js">
    {proExample}
  </Snippet>
</Section>
